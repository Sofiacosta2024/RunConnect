...
export const entrenamiento = pgTable("ENTRENAMIENTO", {
  ...
  idOrganizador: integer("id_organizador").notNull().references(() => organizador.idOrganizador),
  codigoDeporte: text("codigo_deporte").notNull().references(() => deporte.nombre),
  ...
  codigoNivel: text("codigo_nivel").notNull().references(() => nivelEntrenamiento.nivel),
...
export const mensaje = pgTable(
  "MENSAJE",
  {
    fecha: date("fecha").notNull(),
    hora: time("hora").notNull(),
    codigoEntrenamiento: integer("codigo_entrenamiento").notNull().references(() => entrenamiento.codigoEntrenamiento),
    email: text("email").notNull().references(() => usuario.email),
...
export const participacion = pgTable(
  "PARTICIPACION",
  {
    email: text("email").notNull().references(() => usuario.email),
    codigoEntrenamiento: integer("codigo_entrenamiento").notNull().references(() => entrenamiento.codigoEntrenamiento),
    fechaInscripcion: timestamp("fecha_inscripcion", { withTimezone: true }).notNull(),
  },
